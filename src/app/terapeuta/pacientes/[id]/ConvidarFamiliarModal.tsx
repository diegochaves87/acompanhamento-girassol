...
export default function ConvidarFamiliarModal({ patientId, patientName, guardianName, guardianEmail, guardianPhone }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data, error: err } = await supabase
      .from("family_access")
      .insert({ patient_id: patientId, nome: nome.trim(), email: email.trim().toLowerCase(), relacao: relacao || null })
      .select("invite_token")
      .single();
    setLoading(false);
    if (err || !data) { setError("Erro ao criar convite. Verifique o e-mail e tente novamente."); return; }
  ...
  function copyLink() {
    if (!result) return;
    navigator.clipboard.writeText(result.link).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2500); });
  }
            ...
            {result ? (
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3 p-4 rounded-xl" style={{ backgroundColor: "#F0FFF4" }}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Envie o link abaixo para {nome}.</p>
                  </div>
                </div>
                ...
                <button onClick={() => { setOpen(false); reset(); window.location.reload(); }}
                  className="text-xs text-center transition-opacity hover:opacity-60" style={{ color: "#9CA3AF" }}>
                  Fechar
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div>
                  ...
                  <input type="text" required placeholder="Ex: Maria Silva" value={nome}
                ...
                <div>
                  ...
                  <input type="email" required placeholder="familiar@email.com" value={email}
