...
export default function ConvidarFamiliarModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!nome.trim()) { setError("Preencha o nome do familiar."); return; }
    if (!email.trim() && !guardianPhone) {
      setError("Informe o e-mail ou certifique-se de que o responsável tem telefone cadastrado para receber o link.");
      return;
    }
    ...
    if (!res.ok || !json.invite_token) {
      setError(json.error ?? "Erro ao criar convite. Tente novamente.");
      return;
    }
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>
                    Nome do familiar <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>
                    E-mail{" "}
                    <span className="font-normal text-gray-400">(opcional — o familiar pode informar ao aceitar)</span>
                  </label>
                  <input
                    type="email"
                ...
                {guardianPhone && (
                  <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs" style={{ backgroundColor: "#F0FFF4", color: "#166534" }}>
                    ...
                    Link enviado via WhatsApp para {guardianPhone}
                  </div>
                )}
