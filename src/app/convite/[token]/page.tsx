...
function Logo() {
  return (
    ...
    <img src="/identidade-visual/Logo-Nome-Slogan.png" alt="Acompanhamento Girassol" style={{ height: 56, margin: "0 auto 24px", display: "block" }} />
  );
}
...
export default function ConvitePage({ params }: { params: { token: string } }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const supabase = createClient();
    const { error: authError } = await supabase.auth.signUp({ email: invite!.email, password: senha });
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ backgroundColor: "#FFF7E6" }}>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md text-center">
          ...
          <p className="text-xs text-gray-400 mt-3">Você receberá um e-mail assim que o acesso for liberado.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
        <div className="mb-6 text-center">
          ...
          <p className="text-sm text-gray-500 mt-2 leading-relaxed">
            Crie uma senha para finalizar seu cadastro e começar a acompanhar cada sessão com carinho.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input type="text" readOnly value={invite?.nome ?? ""} className={`${inputCls} bg-gray-50 cursor-not-allowed`} />
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>E-mail</label>
            <input type="email" readOnly value={invite?.email ?? ""} className={`${inputCls} bg-gray-50 cursor-not-allowed`} />
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>Criar senha</label>
            <input type="password" required placeholder="Mínimo 6 caracteres" value={senha}
              onChange={(e) => setSenha(e.target.value)} className={inputCls} />
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>Confirmar senha</label>
            <input type="password" required placeholder="Repita a senha" value={confirmar}
