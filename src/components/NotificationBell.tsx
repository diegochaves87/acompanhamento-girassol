...
function typeConfig(tipo: string): { bg: string; color: string; path: string } {
  switch (tipo) {
    case "cpf_missing":
      return { bg: "#FFF8E1", color: "#FFBA3D", path: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" };
    case "evolution_pending":
      return { bg: "#FFF3E0", color: "#FF8C42", path: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" };
    case "invite_accepted":
      return { bg: "#E8F5E9", color: "#4CAF50", path: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" };
    case "invite_pending":
      return { bg: "#E3F2FD", color: "#2E7BC1", path: "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" };
    case "collab_request":
      return { bg: "#F3E5F5", color: "#8E6CCF", path: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" };
    default:
      return { bg: "#F3F4F6", color: "#6B7280", path: BELL_PATH };
  }
}

function NotifIcon({ tipo }: { tipo: string }) {
  const { bg, color, path } = typeConfig(tipo);
  return (
...
export default function NotificationBell() {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    if (data) { setNotifs(data as unknown as Notif[]); setCount(data.length); }
  }, []);
              ...
              notifs.map((n) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    {n.titulo && <p className="text-[10px] font-semibold text-gray-500 mb-0.5">{n.titulo}</p>}
                    <p className="text-xs text-gray-700 leading-snug">{n.mensagem}</p>
